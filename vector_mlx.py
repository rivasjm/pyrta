...
class ResultsCache:
    ...
    @staticmethod
    def _key(priority_matrix: np.array):
        return hash(tuple(priority_matrix.reshape(-1).tolist()))

    def insert(self, priority_matrix: np.array, results: np.array):
        key = self._key(priority_matrix)
        if key not in self.data:
            self.data[key] = results

    def get(self, priority_matrix: np.array):
        key = self._key(priority_matrix)
        return self.data[key] if key in self.data else None

    def has_results(self, priority_matrix: np.array):
        key = self._key(priority_matrix)
...
def system_priority_matrix(system: System) -> np.array:
    ...
    pm = (priorities <= priorities.T) * (mapping == mapping.T) * np.logical_not(np.eye(len(tasks)))
    return pm
...
class VectorHolisticFPAnalysis:
    ...
    def _analysis(priority_matrix, wcets, periods, deadlines, successors, wcrts, jitters, cache, verbose=False, limit=10):
        ...
        pm = np.array(priority_matrix)  # size (s, t, t)
        ...
        r_max = repeat(np.array(wcrts).reshape(1, t, 1), s)
        ...
        while pm.size > 0:
            ...
            while not np.all(p_mask):
                ...
                # initialize w
                w = repeat(p*wcets.reshape(1, t, 1), pm.shape[0])
                # w = np.zeros_like(r)            # (s, t, 1)
                w_prev = np.full(w.shape, 0)   # (s, t, 1)
...
def extract_scenario_data(data: np.array, scenario: int) -> np.array:
    """
...
def repeat(vector, times):
    """vector is a 3D vector with shape (1, t, 1). Times is a positive number. Return shape (times, t, 1) """
    shape = (times, vector.shape[1], 1)
    res = np.zeros(shape)
